...
{-# INLINE liftInIO #-}
liftInIO :: MonadIO m => (State# RealWorld -> (# State# RealWorld, a #)) -> m a
liftInIO = liftIO . IO
...
-- the reallocated size is less then it results in a truncated version of the
-- original array.
--
realloc :: MonadIO m => Int -> Array a -> m (Array a)
realloc i arr = do
    arr1 <- newArray i
    let !newLen@(I# newLen#) = min i (arrLen arr)
        !(I# arrS#) = arrStart arr
    ...
    liftInIO
        $ \s# ->
              let s1# = copyMutableArray# arrC# arrS# arr1C# arr1S# newLen# s#
               in (# s1#, arr1 {arrLen = newLen, arrFree = i} #)
...
snocUnsafe arr@Array {..} a = do
    assert (arrLen + 1 <= arrFree) (return ())
...
toStreamK arr@(Array {..}) = K.mapM (getIndexUnsafe arr) $ K.unfoldr step 0
...
producer = Producer step return return

    where

    {-# INLINE_LATE step #-}
    step Array {..}
        | assert (arrLen >= 0) (arrLen == 0) = return D.Stop
    step arr@(Array {..}) = do
        x <- getIndexUnsafe arr 0
        return $ D.Yield x $ arr {arrStart = arrStart + 1, arrLen = arrLen - 1}
